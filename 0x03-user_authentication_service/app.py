...
def register_user() -> str:
    """Registering a new user if it does not exist before"""
    try:
...
def log_in() -> str:
    """ Logging in a user and returns session ID """
    try:
        email = request.form['email']
        password = request.form['password']
    except KeyError:
...
def log_out() -> str:
    """Finding the user with the requested session ID.
...
def profile() -> str:
    """ In case the user exist, respond with a 200 HTTP status and a JSON Payload
...
def reset_password() -> str:
    """In case the email is not registered, respond with a 403 status code.
